  ...
  class Chat {
    ...
    toggle() {
      for (const key of Object.keys(ChatData)) {
        if (key !== this._element.id && ChatData[key]._isShown) ChatData[key].toggle();
      }
    ...
    _sendMessage() {
      ...
      for (const item of inputs) {
        ...
        if (item.value) {
          parameters += item.name.replace(/^./, item.name[0].toUpperCase()) + ': ' + item.value + '%0A';
        }
      }
      if (valid) {
    ...
    _buildInputs(inputs) {
      ...
      for (const input of inputs) {
        ...
        if (input.placeholder) {
          newInput.placeholder = input.placeholder.replace(/^./, input.placeholder[0].toUpperCase());
        }
    ...
    _setEventListener() {
      ...
      this._contentElement = this._element.querySelectorAll('.' + CLASS_NAME_WIDGET_CONTENT).item(0);
    ...
    _formValidation(formElement) {
      ...
      if (formElement.required && (formElement.value == '' || formElement.value == undefined)) {
    ...
    _typeCheck(config, configTypes) {
      for (const property of Object.keys(configTypes)) {
        ...
        const valueType = value && this._isElement(value) ? 'element' : this._toType(value);
        if (!new RegExp(expectedTypes).test(valueType)) {
          throw new Error(`WhatsApp Widget: ` + `Option "${property}" provided type "${valueType}" ` + `but expected type "${expectedTypes}".`);
        }
      }
    }
    _isElement(object) {
